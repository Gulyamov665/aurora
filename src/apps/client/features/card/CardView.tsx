...
const CardView: FC<CardViewProps> = ({ item, open, setIsOpen, count, setCount }) => {
  ...
  useEffect(() => {
    if (item?.options?.variants.length) {
      const firstActive = item.options.variants.find((v) => v.is_active);
      setSelectedVariant(firstActive?.id ?? null);
    }
  }, [item]);

  useEffect(() => {
    if (selectedVariant && item) {
      const foundOption = item.options?.variants.find((v) => v.id === selectedVariant);
